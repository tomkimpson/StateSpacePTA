...
\begin{abstract}	
	 ...
	 Astrophysically representative software injections in Gaussian measurement noise evidence that the inclusion of the pulsar terms eliminates the biases in the parameter estimates, increases the parameter accuracy, and decrease the minimal detectable strain relative to the previous approach.
	 
	 
	 For a loud GW source with characteristic wave strain $h_0 = 10^{-12}$ the root-mean-square relative error in the estimated one-dimensional posteriors decreases by up to 99\% by including the pulsar terms. 
	 
	 
	 For a quiet GW source with $h_0 = 5 \times 10^{-15}$ since the system is heavily noise dominated.
	 
	 However, including the pulsar terms decreases the minimal detectable strain 
	 
	 

	by \textcolor{red}{Y percent} .
\end{abstract}

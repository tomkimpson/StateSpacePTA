...
The paper is organised as follows. In Section \ref{sec:2} we briefly review the state-space model of PTA data analysis introduced in \cite{KimpsonPTA}. In Section \ref{sec:pulsar_term} we show how to include the pulsar term in the model via a convenient reparametrisation. In Section \ref{sec:rep_example1} we test the updated model which includes the pulsar term on synthetic data for a single representative GW source. In Section \ref{sec:earth_vs_psr} we compare the parameter estimation accuracy and the minimal detectable GW strain when including the pulsar terms to the case where the pulsar terms are dropped. In Section \ref{sec:parameter_space} we extend the tests of Section \ref{sec:rep_example1} to cover an astrophysically relevant domain of SMBHB source parameters. Conclusions are drawn in Section \ref{sec:discussion}. Throughout the paper we adopt the natural units, with $c = G = \hbar = 1$, and metric signature $(-,+,+,+)$. \newline 
...
\begin{equation}
	\chi^{(n)} = \Omega \left( 1 + \boldsymbol{n}\cdot \boldsymbol{q}^{(n)} \right)  d^{(n)} \mod 2 \pi 
\end{equation}
...
\section{Representative example: single SMBHB source including the pulsar terms}\label{sec:rep_example1}
In this section we apply the analysis scheme of K24 while including the pulsar terms using the parametrisation described in Section \ref{sec:pulsar_term2}. As outlined in Section \ref{sec:pulsar_term}, the analysis scheme consists of a Kalman filter to track the state evolution of the intrinsic pulsar spin frequency which provides a likelihood for nested sampling. Nested sampling then returns estimates of the static parameters of the model and $\mathcal{Z}$. We refer the reader to Appendix X and K24 for details of the Kalman filter - nested sampling scheme. We apply the analysis scheme to a PTA perturbed by a GW from an individual quasi-monochromatic SMBHB source. The analysis of a stochastic background composed of the superposition of multiple sources is more challenging and is postponed to a forthcoming paper. The goal of this section is to validate the analysis scheme when the pulsar terms are included. \newline 


In Section \ref{sec:synthetic_data} we demonstrate how to generate synthetic data, namely noisy frequency time series $f_{\rm m}^{(1)}(t), \dots, f_{\rm m}^{(N)}(t)$, with which to validate the scheme. In Section \ref{sec:pe_and_ms} we apply the analysis scheme to the synthetic data to estimate $\boldsymbol{\theta'}$ and $\mathcal{Z}$. The scheme is validated on multiple noise realisations of the pulsar process noise and the measurement noise in order to test the scheme more strongly and quantify the  irreducible ``cosmic'' variance in the parameter estimates. This section considers only a particular, arbitrary set of injection parameters $\boldsymbol{\theta'}$. Extension to broader parameter spaces is considered in Section X.
...
\end{eqnarray}
where, following K24, we use the empirical timing noise model for MSPs from \cite{Shannon2010ApJ...725.1607S} in order to set physically reasonable values on $\sigma_{\rm TOA}^{(n)}$.  The median $\sigma^{(n)}$ calculated in this way is $\sigma^{(n)} = 5.51 \times 10^{-24} $ s$^{-3/2}$, $\min [ \sigma^{(n)} ] = 1.67 \times 10^{-26}$s$^{-3/2}$ for PSR J0645+5158 and $\max [ \sigma^{(n)} ] = 2.56 \times 10^{-19}$ s$^{-3/2}$ for PSR J1939+2134. Compared to the other 46 pulsars of the array, PSR J1939+2134 is a notable outlier with an estimated $\sigma^{(n)}$ much larger than the median. In practice, in order to avoid setting an overly broad prior on $\sigma^{(n)}$ (see Section \ref{sec:set_priors}) for validation purposes, we artificially set $\sigma^{(n)} = 10^{-20} \, {\rm s^{-3/2}}$ for this pulsar, so that $\sigma^{(n)}$ for every pulsar in the synthetic PTA falls within an reasonable width log-uniform prior. This is done purely for testing convenience; it is straightforward to expand the prior when analysing real, astronomical data. \newline 
...
\subsection{Prior distribution}\label{sec:set_priors}
...
\section{Challenges of the pulsar term}
In this section....

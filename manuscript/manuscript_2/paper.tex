...
%
%However, as discussed in K23, First, Equation \ref{eq:frequency_evolution} excludes particular phenomenological elements, which are likely to be present in reality, e.g.\ the classic, two-component, crust-superfluid structure inferred from post-glitch recoveries \citep{Baym1969,vanEysden,Alpar2017MNRAS.471.4827G,Myers2021MNRAS.502.3113M,Meyers2021}. Second, the white spectrum assumed for $\xi(t)$ may or may not be suitable for millisecond pulsars in PTAs. It is challenging observationally to infer the spectrum of $\xi(t)$ from the observed spectrum of the phase residuals, because the inference is conditional on the (unknown) dynamical model governing $df_{\rm p}/dt$. For small-amplitude fluctuations sampled relatively often, as in millisecond pulsars in PTAs, it is likely that $\xi(t)$ is white to a good approximation over the inter-TOA intervals and generates red phase residuals as observed, but caution is warranted nevertheless. White noise fluctuations in $\xi(t)$ translate into red noise fluctuations in the rotational phase $\phi(t) = \int_{t_1}^t dt' \, f_{\rm p}(t')$ after being filtered by the terms involving $d/dt$ and $\gamma$ in Equation \eqref{eq:spinevol}, consistent with the observed power spectral density of typical millisecond pulsars in the nHz band relevant to PTA experiments. Third, $\xi(t)$ is not differentiable, which makes the formal interpretation of $d^2 f_{\rm p} / dt^2$ ambiguous, even though $d^2 f_{\rm p} / dt^2$ is not used in the PTA analysis proposed in this paper. Finally, the Brownian increment $dB(t)=\xi(t)dt$ does not include non-Gaussian excursions such as L\'{e}vy flights \citep{Sornette2004}, which have not been ruled out by pulsar timing experiments to date. The above three idealizations are supplemented by other, physical approximations noted above, e.g.\ neglecting $n_{\rm em}$ in Equation \eqref{eq:spinevol} and differential rotation between the crust and superfluid in Equation \eqref{eq:frequency_evolution}.
...
%The measurement equation is a key input into the Kalman filter in Section \eqref{sec:kalman_filter}
...
%\end{equation}
%with spatial coordinates $\boldsymbol{x}$ and global coordinate time $t$. The GW has a constant angular frequency $\Omega$, propagates in the $\boldsymbol{n}$-direction (where $\boldsymbol{n}$ is a unit vector), has amplitude tensor $H_{\mu \nu}$, and has a phase offset  $\Phi_0$. Throughout this paper we work with 
%
%
% We are free to choose our coordinate system such that $\Phi_0$ is the GW phase at $t=0$ at the SSB. In this paper $\Omega$ has no time dependence; the source is approximated as monochromatic. 
...
%In general radio pulses from a pulsar are transmitted as amplitude modulations of a radio-frequency carrier wave. They are described by the geometric object $\boldsymbol{p}$ which we identify sas the 4-momentum of the radio pulse. The presence of a GW induces a shift in the temporal component of the covariant 4-momentum, while the photon travels from the emitter to the observer, i.e. $\Delta p_t = p_t|_{\rm observer} - p_t|_{\rm emitter} $. One obtains \citep[e.g.][]{Maggiore}
%\begin{equation}
...
%
%The model  and Section \ref{sec:psr_measured} depends on a set of static parameters $\boldsymbol{\theta}$.
%
%
%
%
%
%The model outlined in Section \ref{sec:psr_frequency} and Section \ref{sec:psr_measured} depends on a set of static parameters $\boldsymbol{\theta}$.
%
%
%
%The set of static parameters $\boldsymbol{\theta}$ of the model outlined in Section \ref{sec:model} can be separated into parameters controlling t and the GW source, i.e. 
%\begin{equation}
% \ , \label{eq:params1}
%\end{equation}
%with
...
%and
%
%where $\delta$, $\alpha$, and $\iota$ are the declination, right ascension and inclination of the GW source respectively. In Equation \eqref{eq:params3} we reparameterize the two GW polarisation amplitudes, $h_{+}$ and $h_{\times}$, in terms of $\iota$ and the characteristic wave strain $h_0$ through 
%\begin{align}
%	h_+ &= h_0(1 + \cos^2 \iota) 	\label{eq:hphx} \ ,\\
%	h_{\times} &= -2h_0\cos \iota 	\label{eq:hphx2} \ .
%\end{align}
%We use the parametrisation in terms of $h_0$ and $\iota$ throughout the remainder of this work. 
...
In order to include the pulsar terms in an inference model and apply the methods of K23 it is necessary to be able to handle the noisy likelihood curves presented in Section \ref{sec:pulsar_terms_challenge}. In this section we introduce a new parametrisation of the model described in Section \ref{sec:model} in order to alleviate the noisy likelihoods. \newline 
...
This parametrisation in terms of $\chi^{(n)}$ treats the pulsar dependent phase correction as an explicit parameter to be inferred for every pulsar. The advantage of this approach is that the likelihood curves $\mathcal{L}(\alpha),\mathcal{L}(\delta),\mathcal{L}(\Omega)$ become smooth and resemble the right-hand panel of Figure \ref{fig:likelihood_comparison}. This enables sampling algorithms to efficiently find the likelihood maxima and estimate $p(\boldsymbol{\theta} | \boldsymbol{Y})$. The new parametrisation also does not change the dimension of the parameter space, since we have traded $d^{(n)}$ for $\chi^{(n)}$.
...
\section{Exploring a broader parameter space}

In this section we focus on pulsar terms exclusively and no longer compare with the Earth terms
...
Whilst computational methods for approximate inference from noisy likelihoods do exist \citep[e.g.][]{doi:10.1137/21M1410853,2021arXiv210403942J,2019arXiv190501252J}, 

...
iota and h0 are offset/ not symmetric about the injection value
...
The injected value is contained within the 90\% credible interval of the one-dimensional marginalized posteriors in 45 out of the 63 possible combinations of the seven parameters and 63 realizations (i.e. 70 $\%$). 


Of the 18 posteriors which do not contain the injected value within the  90\% credible interval, 12 



This offset is due to the weak-identifiability of $\iota$-$h$
...
There is appreciable natural random dispersion in the one-dimensional posterior medians between noise realisations. The minimum coefficient of variation is  0.2\% for $\Omega$ , the maximum value is 43\% for $\Phi_0$. The remaining parameters have typical coefficients $\lessapprox 10 \%$. For this representative example, with characteristic strain $h_0 = 5 \times 10^{-15}$ the analysis scheme estimates $\boldsymbol{\theta}_{\rm gw}$ accurately. Analogous results for synthetic data with higher strain ($h_0 = 10^{-12}$) are presented in Appendix \ref{sec:app_high_SNR} (Figure \ref{fig:corner_high_snr_appendix}). At higher strains the dispersion between noise realisations is reduced. \newline 
 ...
 Moreover the systematic offset in $\iota$ in $h_0$ is reduced and becomes symmetric about the injection value 
 
 
 
 
  \textcolor{red}{TK: there is an obvious additional feature that I haven't yet mentioned in the text, which is the systematic offset in iota and h w.r.t the injection. The systematic-ness makes me think this is likely a sampler setting. I am currently running some checks, and will then add few sentences discussing this, or update the results/plots. This is not an issue in the high SNR case: can noise drive results away from the injection systematically? Naively I expect a scatter, symmetric about the injection... How is this related to the chi vals}
...
\begin{table}
	...
	%	\resizebox{\textwidth}{!}{%
		...
		\begin{tabular}{lclll}
			...
			\hline
			\multirow{7}{2mm}{Low SNR} & $\Omega$       & 2.7 &1.6& \hspace{5mm} 1.09 \\

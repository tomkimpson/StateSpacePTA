...
Initially we consider a single noise realisation of the synthetic data $\boldsymbol{Y}$ for the representative example system described in Table \ref{tab:parameters_and_priors}. We consider a "high-SNR" system with $h_0 = 10^{-12}$ and a ``low-SNR" system with $h_0 = 5 \times 10^{-15}$. We apply the Kalman filter using Equation \eqref{eq:measuremen_earth} in conjunction with nested sampling in order to estimate $p(\boldsymbol{\theta} | \boldsymbol{Y})$ for both the high/low SNR systems. Priors on $\boldsymbol{\theta}$ are selected as follows. For $\pi(\boldsymbol{\theta}_{\rm gw})$ we choose standard non-informative priors \citep[e.g.][]{Bhagwat2021} as summarised in Table \ref{tab:parameters_and_priors}. For $\pi(\boldsymbol{\theta}_{\rm psr})$ we use the same priors discussed in K23. The addition here is the prior on $\chi^{(n)}$ which we take to be $\text{Uniform}(0,2 \pi)$. 	All the injected static parameters and their corresponding priors for this representative example are summarised in Table \ref{tab:parameters_and_priors} 
	
The results for the seven parameters of  $\boldsymbol{\theta}_{\rm gw}$ are shown in Figure \ref{fig:corner_plot_1} for the high-SNR system and in Figure \ref{fig:corner_plot_2} for the low-SNR system. 


The histograms shown in the figures are the one-dimensional posteriors for each parameter, marginalized over the set of all other parameters, where the solid orange line indicates the true injection value. The two-dimensional contours denote the (0.5, 1, 1.5, 2)-sigma equivalent contours. The green curves are the results derived using the Earth-terms measurement equation, Equation \ref{eq:g_func_trig_earth}. The blue curves are the results derived using the full measurement equation, including the pulsar terms, Equation \eqref{eq:g_func_trig}



For the high-SNR case, Figure \ref{fig:corner_plot_1} the one-dimensional posteriors for the Earth-terms solution are evidently biased, as was observed in K23. Particular biases are observed in $\Phi_0, \psi, \iota $ and $\alpha$, with the blue posterior displaced with respect to the orange injection line. 

The inclusion of the pulsar terms corrects for this bias. The green posteriors 


Moreover, the injection values are contained witinin the 2-sigma equivalent contours for every pair of parameters. This is not true in the Earth-terms case. 
...
les important for PE in low SNR 


but is important generally for detectio nas we will show.
...
We are able to estimate hte static parameters

the inclusion of the pulsar terms general improves the accuracy

the chi terms are also recoverable 





It is evident that for this representative example with characteristic strain $h_0 = 5 \times 10^{-15}$ that we are able to estimate all the static parameters of the system using our method, with the injected value contained within the 90\% credible interval for all of the parameters. \newline 

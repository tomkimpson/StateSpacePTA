...
if __name__=="__main__":
    ...
    #Setup the system
    P   = SystemParameters(h=h,σp=0.0,σm=1e-13,use_psr_terms_in_data=True,measurement_model=measurement_model,heterodyne=True,heterodyne_scale_factor=1e13)# define the system parameters as a dict. Todo: make this a class
    ...
    print("Ideal likelihood given optimal parameters = ", model_likelihood)
    
   
    #Bilby 
    init_parameters, priors = bilby_priors_dict(PTA,P)

    print("Testing KF using parameters sampled from prior")
    params = priors.sample(1)
    model_likelihood, state_predictions,measurement_predictions = KF.likelihood(params)
    print("Non -ideal likelihood for randomly sampled parameters = ", model_likelihood)

...
with open('batch.sh','w') as b: 
    ...
    for i in range(N):
        ...
        arg_name = f"N4000_single_pp_plot_h_{h}_model_{model}_seed_{seed}_omega_{p['omega_gw']}_phi0_{p['phi0_gw']}_psi_{p['psi_gw']}_delta_{p['delta_gw']}_alpha_{p['alpha_gw']}"
        print(arg_name)

...
Add uniform prior vector specifically for the chi vector
"""
def add_to_bibly_priors_dict_chi_K(x,label,init_parameters,priors,k):
...
"""
def add_to_bibly_priors_dict_chi_K_constant(x,label,init_parameters,priors,k):
...
def set_prior_on_state_parameters(init_parameters,priors,f,fdot,σp,γ,d,chi,K,set_parameters_as_known):

    if set_parameters_as_known:
        logging.info('Setting fully informative priors on PSR parameters')

        init_parameters,priors = add_to_bibly_priors_dict_constant(f,"f0",init_parameters,priors)     
        init_parameters,priors = add_to_bibly_priors_dict_constant(fdot,"fdot",init_parameters,priors)    
        init_parameters,priors = add_to_bibly_priors_dict_constant(σp,"sigma_p",init_parameters,priors)  
        init_parameters,priors = add_to_bibly_priors_dict_constant(γ,"gamma",init_parameters,priors)           
        init_parameters,priors = add_to_bibly_priors_dict_constant(d,"distance",init_parameters,priors)          

        for k in range(K):
          
            init_parameters,priors = add_to_bibly_priors_dict_chi_K_constant(chi[k,:],"chi",init_parameters,priors,k) 
        ...
        for k in range(K):
            init_parameters,priors = add_to_bibly_priors_dict_chi_K(chi[k,:],"chi",init_parameters,priors,k) #uniform


    return init_parameters,priors 


def set_prior_on_measurement_parameters_K(init_parameters,priors,P,set_parameters_as_known):
    ...
    if set_parameters_as_known: #don't set a prior, just assume these are known exactly a priori
        ...
        for k in range(P.num_gw_sources):
            ...
            #Add all the GW quantities
            init_parameters[f"omega_gw_{k}"] = None
            priors[f"omega_gw_{k}"] = bilby.core.prior.LogUniform(1e-7, 1e-6, 'omega_gw')
            ...
            init_parameters[f"h_{k}"] = None
            priors[f"h_{k}"] = bilby.core.prior.LogUniform(5e-15, 1e-14, 'h')
...
def bilby_priors_dict(PTA,P,set_parameters_as_known=False):
    ...
    #Measurement priors
    init_parameters,priors = set_prior_on_measurement_parameters_K(init_parameters,priors,P,set_parameters_as_known) 

    #State priors
    init_parameters,priors = set_prior_on_state_parameters(init_parameters,priors,PTA.f,PTA.fdot,PTA.σp,PTA.γ,PTA.d,PTA.chi,P.num_gw_sources,set_parameters_as_known)

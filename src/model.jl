...
function Q_function(γ::Matrix{NF},n::Matrix{NF},f0::Matrix{NF},dt::NF,σp::NF,n_GW_parameters::Int64) where {NF<:AbstractFloat}

    
    coefficient = NF(2) .*γ .*n .*f0.^(n.-1)
    exponential_term = exp.(-coefficient.*dt) .- NF(1.0)
    
    Q = -(σp)^2 .* exponential_term ./ coefficient #this is just for the pulsars 

    add_on = zeros(NF,1,n_GW_parameters)
    Qout = [Q add_on]


    return diagm(Qout[1,:]) #https://stackoverflow.com/questions/69609872/how-to-make-a-diagonal-matrix-from-a-vector

...
%DIF DEL old_tex_file.tex   Thu Sep 14 08:26:36 2023
%DIF ADD paper.tex          Thu Sep 14 11:10:22 2023
%
...
In general radio pulses from a pulsar are transmitted as amplitude modulations of a radio-frequency carrier wave. They are described by the geometric object $\boldsymbol{p}$ which we \DIFdelbegin \DIFdel{can }\DIFdelend identify  as the 4-momentum of the radio photon. The presence of a GW induces a shift in the temporal component of the covariant 4-momentum\DIFdelbegin \DIFdel{between the emitter and }\DIFdelend \DIFaddbegin \DIFadd{, while the photon travels from the emitter to }\DIFaddend the observer, i.e. $\Delta p_t = p_t|_{\rm observer} - p_t|_{\rm emitter} $\DIFdelbegin \DIFdel{, as \mbox{%DIFAUXCMD
\citep[e.g.][]{Maggiore}
}\hskip0pt%DIFAUXCMD
...
\end{eqnarray}
For \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend synthetic NANOGrav PTA \DIFaddbegin \DIFadd{depicted in Figure \ref{fig:pulsar_distrib}}\DIFaddend , the median $\sigma^{(n)}$ calculated in this way is $\sigma^{(n)} = 4.28 \times 10^{-21} $ s\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{$^{-5/2}$, $\min [ \sigma^{(n)} ] = 1.3 \times 10^{-23}$s$^{-5/2}$ for PSR J0645+5158 and $\max [ \sigma^{(n)} ] = 2.0 \times 10^{-16}$ s$^{-5/2}$ for PSR J1939+2134 }\DIFaddend \newline 

As a \DIFdelbegin \DIFdel{sanity check, we can also }\DIFdelend \DIFaddbegin \DIFadd{cross-check, we }\DIFaddend calculate $\sigma^{(n)}$ \DIFdelbegin \DIFdel{using a complementary numerical approach to the empirical best-fit model outlined above. We can directly solve the state equation }\DIFdelend \DIFaddbegin \DIFadd{by solving }\DIFaddend Equation \eqref{eq:spinevol} numerically using the \texttt{baboo} package \footnote{\url{https://github.com/meyers-academic/baboo}}\DIFdelbegin \DIFdel{so as to obtain }\DIFdelend \DIFaddbegin \DIFadd{, generating }\DIFaddend a synthetic phase solution,
\begin{eqnarray}
...
for a successful example of this approach}\DIFaddend . We obtain empirical phase residuals from the UTMOST pulsar timing program \citep{UTMOST} \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{conducted with }\DIFaddend the Molonglo Observatory Synthesis Telescope \citep{Bailes2017PASA...34...45B}. \DIFdelbegin \DIFdel{For the purposes of validating the $\sigma^{(n)}$ inferred from Equation \eqref{eq:sigmap_f}, it is sufficient at this stage to simply compare the synthetic and real phase residuals visually. Whilst it would also be possible to e.g. define some quantitative loss function (e.g. mean-squared error) between the two solutions and solve the optimisation problem to infer $\sigma^{(n)}$, for our purposes this is excessive. It is satisfactory to simply qualitatively compare the two solutions as a useful sanity check. We emphasise that we are not overly concerned with calculating the maximally accurate }\DIFdelend \DIFaddbegin \DIFadd{A visual cross-check is sufficient for the purposes of this paper, where we seek broadly representative }\DIFaddend values for $\sigma^{(n)}$\DIFdelbegin \DIFdel{, but instead some reasonable values for constructing representative synthetic data . This approach of generating synthetic phase solutions and visually comparing with empirical solutions is also followed in \mbox{%DIFAUXCMD
\cite{Vargas}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{. In an analysis involving real PTA data, $\sigma^{(n)}$ would be estimated from the data jointly with the other static parameters in ${\boldsymbol{\theta}}$}\DIFaddend . In Figure \ref{fig:qualitative_compare} we compare the synthetic and empirical residuals for \DIFdelbegin \DIFdel{the NANOGrav pulsar }\DIFdelend \DIFaddbegin \DIFadd{PSR }\DIFaddend J0030+\DIFdelbegin \DIFdel{0451. We can }\DIFdelend \DIFaddbegin \DIFadd{0451, one of the 47 NANOGrav pulsars plotted in Figure \ref{fig:pulsar_distrib}. We }\DIFaddend see that the synthetic and empirical residuals are qualitatively similar\DIFdelbegin \DIFdel{, and comparable }\DIFdelend \DIFaddbegin \DIFadd{. Comparable }\DIFaddend behaviour is observed for other pulsars in our array.
\begin{figure}
	\includegraphics[width=\columnwidth]{images/example_residuals_plot}
	\caption{Actual (left \DIFdelbeginFL \DIFdelFL{panels}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panel, blue points}\DIFaddendFL ) and synthetic (right \DIFdelbeginFL \DIFdelFL{panels}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{panel, orange points}\DIFaddendFL ) phase residuals for NANOGrav millisecond pulsar \DIFaddbeginFL \DIFaddFL{PSR }\DIFaddendFL J0030+0451. The actual residuals are \DIFdelbeginFL \DIFdelFL{obtained via }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{sourced from the }\DIFaddendFL UTMOST pulsar timing program \citep{UTMOST}. The synthetic residuals are generated by numerically solving Equation \eqref{eq:spinevol} with $\gamma^{(n)} = 10^{-13}$ and $\sigma^{(n)}$ inferred from Equations \eqref{eq:sigmap} and \eqref{eq:sigmap_f}. \DIFdelbeginFL \DIFdelFL{For all pulsars }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The error bars indicate }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{two solutions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{uncertainty in the phase residuals and }\DIFaddendFL are \DIFaddbeginFL \DIFaddFL{generated by propagating the uncertainty in the TOAs through }{\sc \DIFaddFL{tempo2}}\DIFaddFL{. Following \mbox{%DIFAUXCMD
\citet{Vargas} }\hskip0pt%DIFAUXCMD
we set the TOA uncertainty to be a constant $10^{-4}$ s. The blue and orange residuals are }\DIFaddendFL qualitatively similar \DIFdelbeginFL \DIFdelFL{, with comparable magnitudes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by inspection. The other 46 pulsars }\DIFaddendFL in the \DIFdelbeginFL \DIFdelFL{residuals}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synthetic PTA in Section \ref{sec:synt_pta} are tuned to produce comparable results}\DIFaddendFL .}

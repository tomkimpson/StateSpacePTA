...
%DIF DEL old.tex   Thu Oct 26 08:46:55 2023
%DIF ADD new.tex   Thu Oct 26 16:22:55 2023
%
...
% Keep the title short and informative.
	\title[Kalman PTA]{Kalman tracking and \DIFaddbegin \DIFadd{parameter }\DIFaddend estimation of continuous gravitational waves with a pulsar timing array}
...
\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend paper is organised as follows. In Section \ref{sec:model} we present the state-space model for the rotational states of an array of pulsars falling freely in the curved spacetime of a single-source GW. In Section \ref{sec:detect} we develop a Kalman filter to track the state evolution and deploy the Kalman filter in conjunction with nested sampling to estimate the GW and other system parameters, along with the model evidence. In Section \ref{sec:testing} we describe how we create synthetic validation data \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend which to test the method. We go on in Section \ref{sec:rep_example} to test the method on the synthetic data for a single representative GW source. In Section \ref{sec:parameter_space} we extend the tests to cover an astrophysically relevant \DIFdelbegin \DIFdel{parameter space}\DIFdelend \DIFaddbegin \DIFadd{domain of SMBHB source parameters}\DIFaddend . In Section \ref{sec:bias_and_identifiability} we discuss identifiability of and bias in the parameter estimates. \DIFdelbegin \DIFdel{Discussion on  future extensions and conclusions are made in Sections \ref{sec:discussion}and \ref{sec:conclusion} respectively}\DIFdelend \DIFaddbegin \DIFadd{Conclusions are drawn in Section \ref{sec:discussion}}\DIFaddend . The data are formulated as pulse frequency time series with Gaussian measurement noise as a proof of principle and to maintain consistency with previous work \citep{Myers2021MNRAS.502.3113M,Meyers2021}. It will be necessary to modify the method to accept pulse TOAs instead of a pulse frequency time series when implementing it to analyze real data, a subtle generalization which is deferred to future work. Throughout the paper we adopt the natural units, with $c = G = \hbar = 1$, and metric signature $(-,+,+,+)$. \newline 
...
In this paper \DIFdelbegin \DIFdel{$\Omega$ has no time dependence; }\DIFdelend the source is approximated as monochromatic. \DIFdelbegin \DIFdel{Studies of SMBHB inspirals in the PTA context show that the }\DIFdelend \DIFaddbegin \DIFadd{In reality the }\DIFaddend gravitational wave frequency $f_{\rm gw}$ ($=\Omega / 2 \pi $) \DIFdelbegin \DIFdel{changes }\DIFdelend \DIFaddbegin \DIFadd{increases }\DIFaddend during the inspiral by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.][]{Zhu10}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{an amount \mbox{%DIFAUXCMD
\citep[e.g.][]{Sesana2010}
}\hskip0pt%DIFAUXCMD
}\DIFaddend \begin{equation}
	\Delta f_{\rm gw} = \DIFdelbegin \DIFdel{3.94 }\DIFdelend \DIFaddbegin \DIFadd{0.05 }\DIFaddend \, \mathrm{nHz}\left(\DIFdelbegin \DIFdel{\frac{M_{\rm c}}{10^9 M_{\odot}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{M_{\rm c}}{10^{8.5} M_{\odot}}}\DIFaddend \right)^{5 / 3}\left[\DIFdelbegin \DIFdel{\frac{f_{\rm gw}(t=t_1)}{10^{-7} \mathrm{~Hz}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{f_{\rm gw}(t=t_1)}{50 \mathrm{~nHz}}}\DIFaddend \right]^{11 / 3}\left(\frac{T_{\mathrm{obs}}}{10 \mathrm{yr}}\right) \ ,
	\label{eq:f_evolution}
\end{equation}
%DIF > 
%DIF > \begin{equation}
%DIF > 	\Delta f_{\rm gw} = 3.94 \, \mathrm{nHz}\left(\frac{M_{\rm c}}{10^9 M_{\odot}}\right)^{5 / 3}\left[\frac{f_{\rm gw}(t=t_1)}{10^{-7} \mathrm{~Hz}}\right]^{11 / 3}\left(\frac{T_{\mathrm{obs}}}{10 \mathrm{yr}}\right) \ ,
%DIF > 	\label{eq:f_evolution}
%DIF > \end{equation}
where $M_{\rm c}$ is the chirp mass of the SMBHB, $f_{\rm gw}(t=t_1)$ is the GW frequency at the time of the first observation, and $T_{\rm obs}$ is the length of time over which $\Delta f_{\rm gw}$ is measured, which for PTAs is $\sim 10$ years. A source can be considered monochromatic, if $\Delta f_{\rm gw}$ is less than the PTA frequency resolution $1/T_{\rm obs}$. Equation \eqref{eq:f_evolution} implies \DIFdelbegin \DIFdel{$(M_{\rm c}/10^9 M_{\odot})^{5/3} [ f_{\rm gw}(t=t_1) / 10^{-7} \mathrm{~Hz}]^{11/3} (T_{\rm obs}^2/10 \mathrm{yr}) \gtrsim 8 \mathrm{yr}$ }\DIFdelend \DIFaddbegin \DIFadd{$(M_{\rm c}/10^{8.5} M_{\odot})^{5/3} [ f_{\rm gw}(t=t_1) / 50 \mathrm{~nHz}]^{11/3} (T_{\rm obs}/10 \mathrm{yr})^2 \gtrsim 20$ }\DIFaddend for $\Delta f_{\rm gw} \gtrsim 1/T_{\rm obs}$. The majority of SMBHBs detectable with PTAs are expected to satisfy $\Delta f_{\rm gw} < 1/T_{\rm obs}$; for a PTA composed of pulsars with a mean distance of 1.5 kpc, 78\% of simulated SMBHBs satisfy this condition for the current IPTA, whilst for the second phase of the Square Kilometer Array this fraction drops to 52\%; see Figure 7 in  \cite{Rosado10.1093/mnras/stv1098}. We are therefore justified in treating the GW source as monochromatic as a first pass in this introductory paper \citep{Sesana10,Sesana2010,Ellis2012ApJ}. \newline 
...
\begin{figure*}
	...
	\caption{Sample output of the Kalman filter illustrating the accuracy of the reconstructed state sequence $f_{\rm p}^{(1)}(t)$ when the static parameters are correct (${\boldsymbol{\hat\theta}} = {\boldsymbol{\theta}}$, left column) and incorrect (${\boldsymbol{\hat\theta}} \neq {\boldsymbol{\theta}}$, right column). The top panels show the true pulsar state $f_{\rm p}^{(1)}(t) - f_{\rm em}^{(1)}(t)$ (blue curve) and the state estimated by the Kalman filter $\hat{f}_{\rm p}^{(1)}(t) - f_{\rm em}^{(1)}(t)$  (green curve). We subtract $f_{\rm em}^{(1)}(t)$ to better illustrate the stochastic wandering of the pulsar frequency. In the left column, the blue/green solutions overlap almost perfectly; in the right column, they do not. The middle panels show the true measured frequency $f_{\rm m}^{(1)}(t) - f_{\rm em}^{(1)}(t)$ (red curve) and the frequency estimated  by the Kalman filter $\hat{f}_{\rm m}^{(1)}(t) - f_{\rm em}^{(1)}(t)$ (magenta curve). Again we subtract $f_{\rm em}^{(1)}(t)$. In the left column the red/magenta solutions overlap almost perfectly; in the right column, they do not. The bottom panels show the residual or \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL innovation $\epsilon(t) =f_{\rm m}^{(1)}(t) - \hat{f}_{\rm m}^{(1)}(t)$. \DIFdelbeginFL \DIFdelFL{The results are shown for a single pulsar. }\DIFdelendFL In the right column $\Omega$ is displaced from its true value by 20\%. \DIFaddbeginFL \DIFaddFL{Results are shown for a single pulsar. }\DIFaddendFL } 
...
In this section we apply the analysis scheme in Section \ref{sec:detect} and the validation procedure in Section \ref{sec:testing} to a PTA perturbed by a GW from an individual quasi-monochromatic SMBHB source. The analysis of a stochastic background composed of the superposition of multiple sources is more challenging and is postponed to a forthcoming paper. The goal of this section is to run end-to-end through every step in the analysis for a representative worked example to help the reader implement the scheme and reproduce the results in Sections \ref{sec:parameter_space}--\ref{sec:discussion}. To assist with reproducibility, we present intermediate outputs from each step. In Section \ref{sec:priors} we define the priors chosen on $\boldsymbol{\theta}$. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In Section \ref{sec:earth_psr_terms} we \DIFdelbegin \DIFdel{do this. ..
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{\textcolor{red}{TK to do}
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{define the measurement equation used in the inference model by the Kalman filter. }\DIFaddend In Section \ref{sec:parameter_estim} we apply the workflow in Section \ref{sec:methodsummary} to estimate $\boldsymbol{\theta}$, for a single realisation of the pulsar process noise and the measurement noise. In Section \ref{sec:multiple_noise} we extend the parameter estimation exercise to multiple noise realizations to quantify the irreducible ``cosmic'' variance in the parameter estimates. In Section \ref{sec:detection} we calculate the detectability of the source as a function of $h_0$. \newline


The static GW source parameters $\boldsymbol{\theta}_{\rm gw}$ used for this mock analysis are selected to be astrophysically reasonable and representative. The injected \DIFdelbegin \DIFdel{elements }\DIFdelend \DIFaddbegin \DIFadd{components }\DIFaddend of $\boldsymbol{\theta}_{\rm gw}$ \DIFaddbegin \DIFadd{and $\boldsymbol{\theta}_{\rm psr}$ }\DIFaddend are summarised in the second column of Table \ref{tab:parameters_and_priors}.
\DIFdelbegin \DIFdel{The static pulsar parameters $\boldsymbol{\theta}_{\rm psr}$, described in Section \ref{sec:synt_pta}, are also recorded in Table \ref{tab:parameters_and_priors}. 
}\DIFdelend 
...
The parameters that govern the deterministic evolution of $f_{\rm p}^{(n)}(t)$, namely $f_{\rm em}^{(n)} (t_1)$ and $\dot{f}_{\rm em}^{(n)} (t_1)$, are well-determined by radio timing observations. We identify $f_{\rm em}^{(n)} (t_1)$ and $\dot{f}_{\rm em}^{(n)} (t_1)$ with the pulsar barycentric rotation frequency and its time derivative respectively, as quoted in catalogues \citep{Manchester2005}. For the 12.5 year NANOGrav pulsars the median fractional errors on $f_{\rm em}^{(n)}(t_1)$ and $\dot{f}_{\rm em}^{(n)}(t_1)$ are \DIFdelbegin \DIFdel{$\pm 2.68 \times 10^{-13} \%$ and $\pm 2.31 \times 10^{-3} \%$ }\DIFdelend \DIFaddbegin \DIFadd{$\pm 2.68 \times 10^{-13} $ per cent and $\pm 2.31 \times 10^{-3}$ per cent }\DIFaddend respectively. In this paper we adopt uniform priors on $f_{\rm em}^{(n)}(t_1)$ and $\dot{f}_{\rm em}^{(n)}(t_1)$, which extend $\pm 10^3 \eta_f^{(n)}$ and $\pm 10^3 \eta_{\dot{f}}^{(n)}$ respectively about the central, injected values, where $\eta_f^{(n)}$ and $\eta_{\dot{f}}^{(n)}$ denote the errors quoted in the ATNF Pulsar Database \citep{Manchester2005}. \DIFdelbegin \DIFdel{Adopting more relaxed priors tests the method stringently}\DIFdelend \DIFaddbegin \DIFadd{Wider-than-necessary priors test the method more stringently than narrow priors}\DIFaddend . The results below confirm that the method estimates ${\boldsymbol{\theta}}_{\rm gw}$ accurately, whether the priors are \DIFdelbegin \DIFdel{relaxed }\DIFdelend \DIFaddbegin \DIFadd{wide }\DIFaddend or narrow. \newline 


The pulsar distances $d^{(n)}$ are \DIFdelbegin \DIFdel{not as well constrained as }\DIFdelend \DIFaddbegin \DIFadd{less constrained than }\DIFaddend $f_{\rm em}^{(n)} (t_1)$ and $\dot{f}_{\rm em}^{(n)} (t_1)$, with typical uncertainties $\sim 10 \%$ \citep{Yao2017,Arzoumanian2018ApJS..235...37A}. In this paper, however, we omit the term involving $d^{(n)}$ from Equation \eqref{eq:g_func}, as justified in Section \ref{sec:parameter_estim}, so there is no need to set a prior on $d^{(n)}$. Furthermore we do not set a prior on $\gamma^{(n)}$, because we have $\gamma^{(n)} T_{\rm obs} \sim 10^{-5}$, and $\gamma^{(n)}$ is effectively ``unobservable" over a decade; that is, for \DIFdelbegin \DIFdel{$T_{\rm obs}$ =10 }\DIFdelend \DIFaddbegin \DIFadd{$T_{\rm obs}=10$ }\DIFaddend years the solution of Equation \eqref{eq:spinevol} is approximately independent of $\gamma^{(n)}$, as long as $[\gamma^{(n)}]^{-1} \gg T_{\rm obs}$ is satisfied. It is therefore sufficient for validation purposes to carry $\gamma^{(n)}$ through the analysis at its injected value. \footnote{We check this empirically in two independent ways for an informal selection of test cases: (i) we set an uninformative prior $\pi\left [\gamma^{(n)} / 1 \, {\rm s^{-1}} \right] \sim$ LogUniform($10^{-15}, 10^{-10}$) , and (ii) we deliberately displace $\gamma^{(n)}$ in the inference analysis from its true, injected value in the synthetic data, e.g. $\gamma^{(n)} = 10^{-14} \text{ s}^{-1}$ versus $10^{-13} \text{ s}^{-1}$, respectively. The results in (i) and (ii) are \DIFaddbegin \DIFadd{found to be }\DIFaddend the same as those reported in Section \ref{sec:rep_example}.} \newline 

Most pulsars in the synthetic PTA have $10^{-22} \leq \sigma^{(n)} / (1 \, {\rm s^{-3/2}}) \leq 10^{-20}$ as calculated from Equations \eqref{eq:sigmap} and \eqref{eq:sigmap_f}. For these pulsars we set an uninformative broad prior $\pi[\sigma^{(n)} / (1 \, {\rm s^{-3/2}})] \sim$ LogUniform($10^{-23}, 10^{-19}$). The sole exception is PSR J1939+2134 which has $\sigma^{(n)} \sim 10^{-16} \text{s}^{-3/2}$. For validation purposes, we artificially set $\sigma^{(n)} = 10^{-20} \, {\rm s^{-3/2}}$ for this pulsar, so that $\sigma^{(n)}$ for every pulsar in the synthetic PTA falls within the \DIFaddbegin \DIFadd{aforementioned log-uniform }\DIFaddend prior. This is done purely for testing convenience; it is straightforward to expand the prior when analysing real, astronomical data. \newline 
...
\begin{equation}
	...
	\end{equation}
	We defer the inclusion of the pulsar terms \DIFaddbegin \DIFadd{in the inference calculation }\DIFaddend to a future paper. As discussed in Section \ref{sec:priors} this choice reduces the dimensionality of the parameter estimation problem \DIFdelbegin \DIFdel{since }\DIFdelend \DIFaddbegin \DIFadd{because }\DIFaddend the measurement equation, Equation \eqref{eq:measurement}, is no longer a function of the pulsar distance.  We stress that the pulsar terms are dropped only when doing Bayesian inference, i.e. from the Kalman filter model that feeds into the nested sampling algorithm. The synthetic data include the pulsar term in full. \newline 
...
\begin{figure*}
	...
	\caption{\DIFdelbeginFL \DIFdelFL{As }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Same as }\DIFaddendFL Figure \ref{fig:corner_plot_1} but for nine realisations of the noise processes, with curves coloured uniquely. The supertitles of the one-dimensional histograms record the posterior median and the 0.16 and 0.84 quantiles of the median realisation. The known, injected value lies within the 90\% credible interval for 58 out of the 63 combinations of seven parameters and nine noise realizations. There is an appreciable dispersion among the peaks of the one-dimensional posteriors\DIFdelbeginFL \DIFdelFL{is appreciable}\DIFdelendFL , with coefficient of variation $\sim10 \%$ across the 63 combinations. A slight skew-bias is apparent in some of the parameters, e.g. $\iota$. Weak correlations can be seen between \DIFdelbeginFL \DIFdelFL{$\Omega - \Phi_0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Omega$ and $\Phi_0$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$\psi-\alpha$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\psi$ }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{$\iota - h_0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\alpha$, and $\iota$ and $h_0$}\DIFaddendFL .} 
...
We see in Figure \ref{fig:bayes} an approximate quadratic relationship $\beta \propto h_0^2$\DIFdelbegin \DIFdel{exists}\DIFdelend . The GW source is detectable with decisive evidence ($\beta \geq 10$) for $h_0 \gtrsim 4 \times 10^{-15}$. Of course, the minimal detectable strain is particular to the system in Table \ref{tab:parameters_and_priors}. It is influenced in general by \DIFdelbegin \DIFdel{e.g. }\DIFdelend $T_{\rm obs}$, ${\boldsymbol{\theta}}_{\rm gw}$, and ${\boldsymbol{\theta}}_{\rm psr}$, as discussed in Section \ref{sec:parameter_space}. Adjusting $\sigma_{\rm m}$ \DIFdelbegin \DIFdel{linearly scales the effective signal-to-noise ratio $h_0 / \sigma_{\rm m}$, such }\DIFdelend \DIFaddbegin \DIFadd{we find }\DIFaddend that an approximate quadratic relationship $\beta \propto h_0^2 / \sigma_{\rm m}^2$ also exists\DIFaddbegin \DIFadd{, where $h_0 / \sigma_{\rm m}$ is the effective signal-to-noise ratio}\DIFaddend . \newline 
...
\begin{figure}
	...
	\caption{Bayes factor (odds ratio) $\beta$ between the competing models $\mathcal{M}_1$ (GW present in data) and $\mathcal{M}_0$ (GW not present in data) at different GW amplitudes, $h_0$, for the representative example in Table \ref{tab:parameters_and_priors}. The horizontal grey dashed line labels an arbitrary detection threshold, $\beta = 10$. The minimum detectable strain\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL for $\beta < 10$, \DIFdelbeginFL \DIFdelFL{is $ 4 \times 10^{-15}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{equals $4 \times 10^{-15}$}\DIFaddendFL . Missing points for \DIFdelbeginFL \DIFdelFL{$\beta\leq 10$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\beta \lesssim 10$ }\DIFaddendFL occur when noise subverts the hierarchical relationship between $\mathcal{M}_0$ and $\mathcal{M}_1$ (see Section \ref{sec:detection}).}
	\label{fig:bayes}
\end{figure}


\section{\DIFdelbegin \DIFdel{Exploring a broader parameter space}\DIFdelend \DIFaddbegin \DIFadd{SMBHB source parameters}\DIFaddend } \label{sec:parameter_space}
\begin{figure}
	\centering
	\includegraphics[width=\columnwidth]{images/pp_plot_new}
	\caption{\DIFaddbeginFL \DIFaddFL{The fraction of injections included within a given credible interval of the estimated posterior, as a function of the credible interval itself (i.e. }\DIFaddendFL PP plot\DIFdelbeginFL \DIFdelFL{for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{). The injections are }\DIFaddendFL 200 simulated GW sources for a subset of \DIFaddbeginFL \DIFaddFL{five }\DIFaddendFL parameters \DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL $\boldsymbol{\theta}_{\rm gw}$, randomly drawn from the prior distributions \DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL Table \ref{tab:parameters_and_priors}. Each coloured \DIFdelbeginFL \DIFdelFL{line }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{curve }\DIFaddendFL corresponds to a different parameter \DIFdelbeginFL \DIFdelFL{of $\boldsymbol{\theta}_{\rm gw}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(see legend)}\DIFaddendFL . The parameters $h_0$ and $\iota$ are fixed at $5 \times 10^{-15}$ and 1.0 rad respectively in order to maintain an approximately constant SNR. The grey shaded contours \DIFdelbeginFL \DIFdelFL{labels }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{label }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{$1,2,3$-}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1\sigma$, $2\sigma$ and 3}\DIFaddendFL $\sigma$ confidence intervals \DIFaddbeginFL \DIFaddFL{given our statistical sample}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Well }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{For parameters with well }\DIFaddendFL estimated posteriors\DIFaddbeginFL \DIFaddFL{, the PP plot curve }\DIFaddendFL should fall along the diagonal \DIFdelbeginFL \DIFdelFL{$y=x$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of unit slope}\DIFaddendFL . $\Omega$ is generally well-estimated \DIFaddbeginFL \DIFaddFL{(i.e. it lies close to the unit diagonal) }\DIFaddendFL but \DIFdelbeginFL \DIFdelFL{many }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the four }\DIFaddendFL other parameters show evidence of being over-constrained \DIFaddbeginFL \DIFaddFL{(i.e. the curves lie above the unit diagonal for low credible intervals, below the unit diagonal for high credible intervals). This is }\DIFaddendFL due to a modelling bias.}
	\label{fig:parameter_space}
\end{figure}
\DIFdelbegin \DIFdel{We have so far focussed }\DIFdelend \DIFaddbegin \DIFadd{Section \ref{sec:rep_example} focuses }\DIFaddend on a single representative system, summarised in Table \ref{tab:parameters_and_priors}. In this section we test the method \DIFdelbegin \DIFdel{in different regions of the parameter space}\DIFdelend \DIFaddbegin \DIFadd{for various systems}\DIFaddend , varying the \DIFaddbegin \DIFadd{SMBHB }\DIFaddend source parameters through astrophysically relevant ranges. \newline 


We \DIFdelbegin \DIFdel{consider }\DIFdelend \DIFaddbegin \DIFadd{analyse }\DIFaddend 200 injections \DIFdelbegin \DIFdel{for different sets of parameter values where we fix }\DIFdelend \DIFaddbegin \DIFadd{constructed by fixing }\DIFaddend $h_0 = 5 \times 10^{-15}$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $\iota =1.0$ and \DIFdelbegin \DIFdel{draw }\DIFdelend \DIFaddbegin \DIFadd{drawing }\DIFaddend the remaining five \DIFdelbegin \DIFdel{parameters }\DIFdelend \DIFaddbegin \DIFadd{elements }\DIFaddend of $\boldsymbol{\theta}_{\rm gw}$ \DIFaddbegin \DIFadd{randomly }\DIFaddend from the prior distributions \DIFdelbegin \DIFdel{described }\DIFdelend \DIFaddbegin \DIFadd{defined }\DIFaddend in Table \ref{tab:parameters_and_priors}. We fix $h_0$ and $\iota$ in order to maintain an approximately constant \DIFdelbegin \DIFdel{SNR across the parameter space}\DIFdelend \DIFaddbegin \DIFadd{signal-to-noise ratio (SNR) across the 200 injections}\DIFaddend . For each \DIFdelbegin \DIFdel{simulated injection we can then attempt to recover the posterior distributions for each parameter}\DIFdelend \DIFaddbegin \DIFadd{injection we compute the posterior distribution of ${\boldsymbol{\theta}}_{\rm gw}$}\DIFaddend . To summarise the results \DIFdelbegin \DIFdel{across the parameter space }\DIFdelend we use a parameter-parameter (PP) plot \citep{doi:10.1198/106186006X136976}. A PP plot \DIFdelbegin \DIFdel{describes }\DIFdelend \DIFaddbegin \DIFadd{displays }\DIFaddend the fraction of \DIFdelbegin \DIFdel{the total number of injected parameters which are }\DIFdelend \DIFaddbegin \DIFadd{injections }\DIFaddend included within a given credible interval of the estimated posterior\DIFdelbegin \DIFdel{with respect to }\DIFdelend \DIFaddbegin \DIFadd{, plotted as a function of }\DIFaddend the credible interval itself. In the ideal case \DIFaddbegin \DIFadd{of perfect recovery, }\DIFaddend the PP plot should be a diagonal line \DIFdelbegin \DIFdel{, indicating that $x$-percentage of the simulated injections fall within the $x$-percentage credible interval. The results are shown in Figure \ref{fig:parameter_space} }\DIFdelend \DIFaddbegin \DIFadd{of unit slope. }\newline 

 \DIFadd{Figure \ref{fig:parameter_space} displays the results of the numerical experiment described in the previous paragraph}\DIFaddend . The shaded grey contours enclose the $1\sigma$, $2\sigma$, and $3\sigma$ significance levels \DIFdelbegin \DIFdel{, given }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend 200 injections. We \DIFdelbegin \DIFdel{can }\DIFdelend see that only $\Omega$ falls \DIFaddbegin \DIFadd{wholly }\DIFaddend within the $3\sigma$ shaded region. The \DIFaddbegin \DIFadd{PP curves for the }\DIFaddend other parameters deviate from the \DIFdelbegin \DIFdel{$y=x$ diagonal . The effect is most }\DIFdelend \DIFaddbegin \DIFadd{diagonal of unit slope. The deviation is more }\DIFaddend pronounced for $\alpha$ and $\delta$ \DIFdelbegin \DIFdel{with more modest deviations }\DIFdelend \DIFaddbegin \DIFadd{and less }\DIFaddend for $\psi$ and $\Phi_0$. The shape of the graph indicates that the posteriors for these parameters are over-constrained; there are fewer injections contained within \DIFdelbegin \DIFdel{high value }\DIFdelend \DIFaddbegin \DIFadd{high-value }\DIFaddend credible intervals than would be expected statistically, and there are more injections contained within the \DIFdelbegin \DIFdel{low value }\DIFdelend \DIFaddbegin \DIFadd{low-value }\DIFaddend credible intervals. This \DIFdelbegin \DIFdel{is an result of }\DIFdelend \DIFaddbegin \DIFadd{stems from }\DIFaddend the bias that we \DIFdelbegin \DIFdel{observed }\DIFdelend \DIFaddbegin \DIFadd{noted }\DIFaddend in Section \ref{sec:multiple_noise}; the posteriors inferred for these parameters are generally confident, but are systematically biased away from the true injection values. This manifests in the PP plot as posteriors which are overly precise (narrow) to contain the injected value within the appropriate credible interval. \DIFdelbegin \DIFdel{An in-depth discussion on the origin of this bias is given }\DIFdelend \DIFaddbegin \DIFadd{The origin of the bias is discussed }\DIFaddend in Section \ref{sec:bias}. 

\section{\DIFdelbegin \DIFdel{Identifiability and bias}\DIFdelend \DIFaddbegin \DIFadd{Systematic biases}\DIFaddend } \label{sec:bias_and_identifiability}
 \DIFdelbegin \DIFdel{There are two underlying issues in the preceding tests that we now explore in more detail}\DIFdelend \DIFaddbegin \DIFadd{Two systematic aspects of the tests in Sections \ref{sec:rep_example} and \ref{sec:parameter_space} merit further study}\DIFaddend . The first relates to parameter identifiability, i.e. can we uniquely identify the parameters of the model given the data and the model structure? This question is explored in Section \ref{sec:identif}. The second relates to a bias in the parameter estimates that results from dropping the pulsar terms from the Kalman filter measurement equation as described in Section \ref{sec:parameter_estim}. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend is the focus of Section \ref{sec:bias}. In order to elucidate these \DIFdelbegin \DIFdel{questions }\DIFdelend \DIFaddbegin \DIFadd{matters }\DIFaddend without confusion from the measurement noise, we \DIFdelbegin \DIFdel{now work in the high SNR }\DIFdelend \DIFaddbegin \DIFadd{switch to the high-SNR }\DIFaddend regime and set $h_0 = 10^{-12}$ (c.f. Figure \ref{fig:bayes}) for \DIFdelbegin \DIFdel{our representative example system (Table \ref{tab:parameters_and_priors})}\DIFdelend \DIFaddbegin \DIFadd{the representative system in Table \ref{tab:parameters_and_priors}}\DIFaddend .
...
\begin{figure}
	...
	\caption{\DIFdelbeginFL \DIFdelFL{Likelihood }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Log-likelihood }\DIFaddendFL $\log \mathcal{L}$ (Equation \ref{eq:likelihood}) \DIFdelbeginFL \DIFdelFL{surface across }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{contours in }\DIFaddendFL the $\iota - h_0$ \DIFdelbeginFL \DIFdelFL{parameter space }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{plane }\DIFaddendFL for a single noise realisation. The red \DIFdelbeginFL \DIFdelFL{point labels }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{dot marks }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{location }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{maximum }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{the likelihood maxima}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\log {\cal L}$}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{likelihood surface has been }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{colour scale is }\DIFaddendFL normalised with respect to the absolute value of the \DIFdelbeginFL \DIFdelFL{maxima}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{maximum}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{A strong }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{An S-shaped }\DIFaddendFL likelihood ridge is \DIFdelbeginFL \DIFdelFL{evident in the parameter space where the Kalman filter produces similar likelihood values}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{visible}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{making }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{which makes }\DIFaddendFL it difficult for \DIFdelbeginFL \DIFdelFL{sampling algorithms }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the nested sampler }\DIFaddendFL to converge to a \DIFdelbeginFL \DIFdelFL{consistent }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{unique }\DIFaddendFL value.}
	\label{fig:likelihood_surface}
\end{figure}
In Figure \ref{fig:just_iota_h} we plot the \DIFdelbegin \DIFdel{estimated }\DIFdelend one-dimensional posteriors for $\iota$ and $h_0$ for \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend high-SNR representative system \DIFdelbegin \DIFdel{, over }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend nine noise realisations. 
The figure is entirely analogous to Figure \ref{fig:corner_plot_2}, but \DIFdelbegin \DIFdel{just for }\DIFdelend \DIFaddbegin \DIFadd{the corner plot displays }\DIFaddend $\iota$ and $h_0$ rather than all \DIFdelbegin \DIFdel{members }\DIFdelend \DIFaddbegin \DIFadd{elements }\DIFaddend of $\boldsymbol{\theta}_{\rm gw}$. \DIFdelbegin \DIFdel{In the case of high-SNR we might expect }\textit{\DIFdel{a priori}} %DIFAUXCMD
\DIFdel{that our estimates would be increasingly accurate over the low-SNR case, and that the method would converge robustly to similar posteriors irrespective of the noise realisation (i.e. $W_1 \to 0$) . This is generally true for the majority of the component parameters of $\boldsymbol{\theta}_{\rm gw}$; }\DIFdelend \DIFaddbegin \DIFadd{One observes four things: (i) the plotted posteriors are unimodal but offset systematically from the true, injected values, which is related to }\DIFaddend the \DIFdelbegin \DIFdel{estimated posteriors across noise realisations are largely indistinguishable. However for $\iota$ and }\DIFdelend \DIFaddbegin \DIFadd{bias discussed in Section 7.2; (ii) a correlation between }\DIFaddend $h_0$ \DIFdelbegin \DIFdel{this is not the case. Instead the inferred one-dimensional posteriors do not broadly overlap and are not consistent across different noise realisations. Specifically}\DIFdelend \DIFaddbegin \DIFadd{and $\iota$ is visible in the two-dimensional contours; (iii) the posteriors do not overlap tightly, a trend seen also in the $\delta$ posterior in Figure \ref{fig:corner_plot_2}; and (iv) features (i)--(iii) occur, even though the SNR is high. One expects a priori that the parameter estimates should become more accurate, as the SNR increases, but this is not true for $h_0$ and $\iota$. Specifically, }\DIFaddend across $10^3$ noise realisations\DIFaddbegin \DIFadd{, }\DIFaddend the median WD for $\iota$ is $W_{1, \rm median}=0.24$ rad and for $h_0$ is  $W_{1, \rm median}=1.6 \times 10^{-13}$. \DIFdelbegin \DIFdel{For $\iota$}\DIFdelend \DIFaddbegin \DIFadd{Both $W_{1,{\rm median}}$ values are higher (as a fraction of the true, injected value) than in the lower-SNR tests conducted in Sections \ref{sec:rep_example} and \ref{sec:parameter_space}. In contrast}\DIFaddend , $W_{1,\rm median}$ \DIFdelbegin \DIFdel{is greater than the value inferred in the low SNR case ($=0.14$, Table \ref{tab:Wasserstein}), whilst for $h_0$ $W_{1,\rm median}$ is also greater as a proportion of the scale set by the strain amplitude. This is in contrast to $W_{1,\rm median}$ inferred }\DIFdelend for the other \DIFdelbegin \DIFdel{parameters of $\boldsymbol{\theta}_{\rm gw}$ in the high-SNR regime where as SNR increases, $W_1$ decreases; e.g. for $\Phi_0$, $W_{1, \rm median} = 4.5 \times 10^{-3}$ rad}\DIFdelend \DIFaddbegin \DIFadd{five parameters in $\boldsymbol{\theta}_{\rm gw}$ decreases, as the SNR increases}\DIFaddend . \newline 

\DIFdelbegin \DIFdel{This issue of inferring very different posteriors depending on the particular realisation of the noise is due to these parameters being only weakly-identifiable. Identifiability }\DIFdelend \DIFaddbegin \DIFadd{We infer different $h_0$ and $\iota$ posteriors for different noise realizations, because $h_0$ and $\iota$ are weakly identifiable. Identifiability is a formal concept from the theory of signal processing, which is applied routinely to engineering problems \mbox{%DIFAUXCMD
\citep{e5be7c83a0d24500826f6e1b414d1733}}\hskip0pt%DIFAUXCMD
. It }\DIFaddend refers to whether it is theoretically possible to infer unique \DIFdelbegin \DIFdel{parameter values of the model}\DIFdelend \DIFaddbegin \DIFadd{and accurate parameter values}\DIFaddend , given the measured data and the model structure \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{e5be7c83a0d24500826f6e1b414d1733}}\hskip0pt%DIFAUXCMD
}\DIFdelend . Sources of non-identifiability are generally categorised as either ``structural\DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{'' }\DIFaddend , arising from the structure of the model \DIFaddbegin \DIFadd{(e.g. its invertibility)}\DIFaddend , or ``practical\DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{''}\DIFaddend , arising from insufficient data or measurement errors \citep{GUILLAUME2019418}. Regarding $\iota$ and $h_0$, \DIFdelbegin \DIFdel{we are concerned with structuralidentifiability. Indeed, one might initially suspect that $\iota$ and $h_0$ could have structural identifiability issues; from }\DIFdelend \DIFaddbegin \DIFadd{the source is structural. }\DIFaddend Equations \eqref{eq:hphx} and \eqref{eq:hphx2} \DIFdelbegin \DIFdel{we can see that there is }\DIFdelend \DIFaddbegin \DIFadd{contain }\DIFaddend a weak degeneracy between $\iota$ and \DIFdelbegin \DIFdel{$h$}\DIFdelend \DIFaddbegin \DIFadd{$h_0$}\DIFaddend . For example, \DIFdelbegin \DIFdel{from Equation \eqref{eq:hphx2} it is not clear if a larger value of }\DIFdelend the cross polarisation strain $h_{\times}$ \DIFdelbegin \DIFdel{is due to the system having a larger strain amplitude }\DIFdelend \DIFaddbegin \DIFadd{involves a trade-off between $\iota$ and }\DIFaddend $h_0$, \DIFdelbegin \DIFdel{or a different inclination $\iota$. This degeneracy is only broken by the relation for $h_{+}$, }\DIFdelend \DIFaddbegin \DIFadd{which is broken by knowing $h_+$ via }\DIFaddend Equation \eqref{eq:hphx}. \DIFdelbegin \DIFdel{It can be shown (Appendix \ref{appendix_identifiability}) that }\DIFdelend \DIFaddbegin \DIFadd{However, the measured strain is a linear combination of $h_\times$ and $h_+$; the two polarizations are not measured independently, which makes it challenging to separate }\DIFaddend $\iota$ and $h_0$\DIFdelbegin \DIFdel{are in fact structurally identifiable, but with the caveat that they are only weakly-identifiable}\DIFdelend \DIFaddbegin \DIFadd{. In fact $\iota$ and $h_0$ are structurally identifiable --- but only weakly}\DIFaddend . That is\DIFdelbegin \DIFdel{to say the $\iota-h_0$ likelihood surface plateaus }\DIFdelend \DIFaddbegin \DIFadd{, the $\iota$-$h_0$ likelihood contours plateau }\DIFaddend to a ridge with a small, non-zero gradient close to the maximum likelihood solution. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend likelihood ridge is presented in Figure \ref{fig:likelihood_surface} for a particular noise realisation. Whilst a single, unique \DIFdelbegin \DIFdel{maxima }\DIFdelend \DIFaddbegin \DIFadd{maximum does }\DIFaddend exists (labelled by the red \DIFdelbegin \DIFdel{point }\DIFdelend \DIFaddbegin \DIFadd{dot }\DIFaddend in the figure), \DIFdelbegin \DIFdel{along the ridge in the parameter space the }\DIFdelend \DIFaddbegin \DIFadd{it lies along an S-shaped ridge where the }\DIFaddend likelihood values are all \DIFdelbegin \DIFdel{very }\DIFdelend similar. Consequently it is difficult for the nested sampling algorithm to \DIFdelbegin \DIFdel{select a particular point in this plateau and instead effectively }\DIFdelend \DIFaddbegin \DIFadd{settle on the red dot; it }\DIFaddend trades off accuracy in one parameter against \DIFdelbegin \DIFdel{accuracy in }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend other, depending on the particular realisation of the noise. \DIFdelbegin \DIFdel{If during the inference }\DIFdelend \DIFaddbegin \DIFadd{By way of verification, if }\DIFaddend we fix one \DIFdelbegin \DIFdel{of the parameters }\DIFdelend \DIFaddbegin \DIFadd{parameter }\DIFaddend at its true value (i.e. set a delta function prior) \DIFdelbegin \DIFdel{, taking a slice through the likelihood surface it is then possible to identify a particular point in the plateau and the }\DIFdelend \DIFaddbegin \DIFadd{during the inference analysis, the }\DIFaddend posteriors for the \DIFaddbegin \DIFadd{other parameter overlap indistinguishably  for }\DIFaddend different noise realisations\DIFdelbegin \DIFdel{overlap indistinguishably}\DIFdelend , i.e. $W_1 \to 0$. 
\DIFdelbegin \DIFdel{In the low-SNR case these parameters are also weakly identifiable, but the increased measurement noise increases the uncertainty which effectively ``washes out" }\footnote{%DIFDELCMD < \tiny %%%
\DIFdel{\textcolor{red}{TK: "washes out" may be too colloquial / unspecific, but I think it is intuitive...}}%DIFDELCMD < \normalsize%%%
}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{the likelihood ridge, leading to a more concave likelihood surface. This enables consistent posteriors to be estimated for $\iota$ and $h$. In the case where we fix e.g $h_0$ at its true value to infer indistinguishable posteriors for $\iota$, all of the posteriors for $\iota$ are all strongly biased away from the injected value; the maximum a posteriori probability estimates underestimate the injected value by $\sim 0.34$ rad. This is a more definitive sign of the bias that was suggested at in Sections \ref{sec:parameter_estim} and \ref{sec:multiple_noise} and will be now be discussed further in Section \ref{sec:bias}. 
}\DIFdelend \DIFaddbegin 


\DIFaddend \subsection{\DIFdelbegin \DIFdel{Bias by dropping }\DIFdelend \DIFaddbegin \DIFadd{Dropping the }\DIFaddend pulsar terms}\label{sec:bias}

\begin{figure*}
	\includegraphics[width=\textwidth, height =\textwidth]{images/large_h_example}
	\caption{\DIFdelbeginFL \DIFdelFL{As }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Same as }\DIFaddendFL Figure \ref{fig:corner_plot_1} but for \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL high-SNR system with $h_0 = 10^{-12}$. For \DIFdelbeginFL \DIFdelFL{the majority }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{five out }\DIFaddendFL of \DIFaddbeginFL \DIFaddFL{seven }\DIFaddendFL parameters the \DIFdelbeginFL \DIFdelFL{injection }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{true, injected }\DIFaddendFL value does not fall within the 90\% credible interval and \DIFdelbeginFL \DIFdelFL{is not visible over these scales}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{falls outside the plotted domain}\DIFaddendFL . The estimated posteriors are biased away from the injected value due to \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL dropping \DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL the pulsar terms. Note that the \DIFdelbeginFL \DIFdelFL{scale of the $x$-axes are generally highly constrained compared to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{plotted domain is narrower than in }\DIFaddendFL Figure \ref{fig:corner_plot_1}.} 
...
\begin{figure}
	...
	\caption{\DIFdelbeginFL \DIFdelFL{Variation in the log likelihood }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Log-likelihood }\DIFaddendFL returned by the Kalman filter \DIFaddbeginFL \DIFaddFL{as a function of inclination $\iota$, }\DIFaddendFL using the Earth terms model (Equation \ref{eq:measuremen_earth}, blue line), the pulsar terms model (Equation \ref{eq:measurement}, orange line), and \DIFaddbeginFL \DIFaddFL{a control experiment that drops }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{Earth }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pulsar }\DIFaddendFL terms \DIFdelbeginFL \DIFdelFL{model for Earth terms only }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{from both the }\DIFaddendFL synthetic data \DIFaddbeginFL \DIFaddFL{and the Kalman filter model }\DIFaddendFL (green line)\DIFdelbeginFL \DIFdelFL{, with respect to the inclination $\iota$, over the prior space}\DIFdelendFL . All other parameters are fixed at their true injected value from Table \ref{tab:parameters_and_priors}. The coloured dashed vertical lines \DIFdelbeginFL \DIFdelFL{label }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mark }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{location of the likelihood maxima }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{maximum }\DIFaddendFL for \DIFdelbeginFL \DIFdelFL{their }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL corresponding model; the green \DIFdelbeginFL \DIFdelFL{/}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL orange dashed lines \DIFdelbeginFL \DIFdelFL{are overlaid on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{overlap at }\DIFaddendFL this \DIFdelbeginFL \DIFdelFL{scale}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{resolution}\DIFaddendFL . For the Earth-terms model the likelihood \DIFdelbeginFL \DIFdelFL{maxima }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{maximum ($\iota \approx 0.7$) }\DIFaddendFL does not coincide with the injected value ($\iota = 1.0$). For the pulsar-terms model the \DIFdelbeginFL \DIFdelFL{location of the maxima }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{maximum }\DIFaddendFL and injected value do coincide. 10 noise realisations are plotted for each situation but \DIFdelbeginFL \DIFdelFL{the solutions are completely overlaid on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{overlap at }\DIFaddendFL this \DIFdelbeginFL \DIFdelFL{scale}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{resolution; they are indistinguishable to the eye}\DIFaddendFL .}
	\label{fig:likelihood_surface_iota}
\end{figure}
\DIFdelbegin \DIFdel{Evident in the preceding discussion is the fact that for some of the $\boldsymbol{\theta}_{\rm gw}$ parameters the inferred value }\DIFdelend \DIFaddbegin 

\DIFadd{Sections \ref{sec:rep_example} and \ref{sec:parameter_space} present preliminary evidence that the inferred value of ${\boldsymbol{\theta}}_{\rm gw}$ }\DIFaddend (e.g. the \DIFdelbegin \DIFdel{median }\DIFdelend \DIFaddbegin \DIFadd{medians }\DIFaddend of the one-dimensional marginalised \DIFdelbegin \DIFdel{posterior}\DIFdelend \DIFaddbegin \DIFadd{posteriors}\DIFaddend ) is biased away from the true \DIFdelbegin \DIFdel{injection value. In Figure \ref{fig:bias_for_large_h} we show the resulting posteriors }\DIFdelend \DIFaddbegin \DIFadd{injected value. Figure \ref{fig:bias_for_large_h} displays a corner plot }\DIFaddend for $\boldsymbol{\theta}_{\rm gw}$, analogous to Figure \ref{fig:corner_plot_1}, but \DIFdelbegin \DIFdel{now for the high-SNR system with $h_0 = 10^{-12}$. We can see that for all parameters, with the exception of }\DIFdelend \DIFaddbegin \DIFadd{for high SNR $(h_0 = 10^{-12})$. Except for }\DIFaddend $\Omega$ and $h_0$, the injected value lies outside the 90\% credible interval, to the extent that it \DIFdelbegin \DIFdel{is not visible on the scales over which we have plotted the posteriors. This effect }\DIFdelend \DIFaddbegin \DIFadd{falls outside the plotted domain. The deviation }\DIFaddend is most severe for $\iota$ with a bias of $\sim 0.3$ rad but is also present to a lesser extent in other static parameters such as $\delta,\alpha$ and $\psi$\DIFdelbegin \DIFdel{, and additionally to an even smaller extent in $\Phi_0$}\DIFdelend . Similar results are obtained for other noise realisations. Whilst $h_0$ does not exhibit any noticeable bias for \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend particular noise realisation \DIFaddbegin \DIFadd{generating Figure \ref{fig:bias_for_large_h}}\DIFaddend , in general \DIFdelbegin \DIFdel{across multiple noise realisations }\DIFdelend it does exhibit a correlated bias with $\iota$ \DIFaddbegin \DIFadd{across multiple noise realisations, as noted in the bottom left panel of Figure \ref{fig:just_iota_h}}\DIFaddend . In this \DIFdelbegin \DIFdel{Section }\DIFdelend \DIFaddbegin \DIFadd{section }\DIFaddend we demonstrate how \DIFdelbegin \DIFdel{this bias results from the dropping of }\DIFdelend \DIFaddbegin \DIFadd{the bias stems from dropping }\DIFaddend the pulsar terms described in Section \ref{sec:parameter_estim}. \newline 


In Figure \ref{fig:likelihood_surface_iota} we plot the \DIFdelbegin \DIFdel{variation in the }\DIFdelend log-likelihood returned by the Kalman filter, \DIFaddbegin \DIFadd{viz. }\DIFaddend Equation \eqref{eq:likelihood}, as \DIFdelbegin \DIFdel{we vary }\DIFdelend \DIFaddbegin \DIFadd{a function of }\DIFaddend $\iota$ \DIFaddbegin \DIFadd{(i.e. $\log \mathcal{L}(\iota)$) }\DIFaddend across the prior \DIFdelbegin \DIFdel{space}\DIFdelend \DIFaddbegin \DIFadd{domain}\DIFaddend , holding all other parameters constant at the true injected value of the representative \DIFdelbegin \DIFdel{example system , }\DIFdelend \DIFaddbegin \DIFadd{system in }\DIFaddend Table \ref{tab:parameters_and_priors}. We \DIFdelbegin \DIFdel{do this for }\DIFdelend \DIFaddbegin \DIFadd{generate }\DIFaddend 10 \DIFaddbegin \DIFadd{$\log \mathcal{L}(\iota)$ solutions corresponding to 10 }\DIFaddend separate realisations of the system noise. \DIFdelbegin \DIFdel{We consider three separate situations: }\textit{\DIFdel{(i)}} %DIFAUXCMD
\DIFdelend The solid blue \DIFdelbegin \DIFdel{line is the likelihood curve with just the Earth-terms included }\DIFdelend \DIFaddbegin \DIFadd{curves only includes the Earth terms }\DIFaddend (i.e. using Equation \DIFdelbegin \DIFdel{\ref{eq:measuremen_earth} }\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:measuremen_earth} }\DIFaddend in the Kalman filter)\DIFdelbegin \DIFdel{, }\textit{\DIFdel{(ii)}} %DIFAUXCMD
\DIFdel{the solid orange line is the likelihood curve with the additional pulsar terms included }\DIFdelend \DIFaddbegin \DIFadd{. The solid orange curves include the Earth and pulsar terms }\DIFaddend (i.e. using Equation \DIFdelbegin \DIFdel{\ref{eq:measurement}), }\textit{\DIFdel{(iii)}} %DIFAUXCMD
\DIFdel{the solid green line is the solution }\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:measurement}). The solid green curve corresponds to a control experiment, }\DIFaddend where we drop the pulsar terms from both the synthetic data and the measurement equation. \DIFdelbegin \DIFdel{We have not used nested sampling for situations }\textit{\DIFdel{(ii)}} %DIFAUXCMD
\DIFdel{or }\textit{\DIFdel{(iii)}} %DIFAUXCMD
\DIFdel{in this work. However, we are able to run the Kalman filter either using the pulsar terms in the model or else dropping them from the synthetic data consider how the resulting likelihood varies for different parameter values. The dashed lines show the location of the likelihood maximafor each of the likelihood solutions; finding the location of the maxima across all static parameters is effectively the goal of any likelihood based inference method, such as nested sampling }\DIFdelend \DIFaddbegin \DIFadd{The vertical dashed lines mark the maxima, which nested sampling seeks}\DIFaddend . \newline  


The key observation from Figure \ref{fig:likelihood_surface_iota} is that \DIFdelbegin \DIFdel{for the Earth-terms only solution the }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend location of the \DIFdelbegin \DIFdel{likelihood optima and the location of the injected value are not the same. Consequently Bayesian likelihood inference techniques settle in optima that do not necessarily align }\DIFdelend \DIFaddbegin \DIFadd{maximum ($\iota \approx 0.7 \, {\rm rad}$) does not coincide }\DIFaddend with the true\DIFdelbegin \DIFdel{injection value }\DIFdelend \DIFaddbegin \DIFadd{, injected value ($\iota = 1.0 \, {\rm rad}$), when only the Earth terms are included}\DIFaddend . This is the origin of the bias in the one-dimensional marginalised posteriors\DIFdelbegin \DIFdel{that we observe. For the Earth-terms solution the optima of the likelihood curve lies around $\iota \sim 0.7$ rad, whereas the injected value was $\iota = 1.0$ rad. This optima is comparable to the median of the posterior inferred for $\iota$ in Figure \ref{fig:bias_for_large_h}. }%DIFDELCMD < \newline 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In contrastto the Earth-terms solution, for the pulsar-terms solution the optima of the likelihood curve now }\DIFdelend \DIFaddbegin \DIFadd{. In contrast, the maximum }\DIFaddend aligns with the true\DIFdelbegin \DIFdel{injected value. The corollary is also true; }\DIFdelend \DIFaddbegin \DIFadd{, injected value, when the pulsar terms are included. As a cross-check, agreement is also achieved, }\DIFaddend if we drop the pulsar terms from the synthetic data \DIFaddbegin \DIFadd{(which is not the situation in reality, of course) }\DIFaddend and use the Earth terms \DIFdelbegin \DIFdel{equation for inferencethen the optima is also correctly located. This evidences that it is the effect of dropping the pulsar terms from the measurement equation which causes this bias in the inferred posteriors. }\DIFdelend \DIFaddbegin \DIFadd{only for inference. }\DIFaddend Similar results are \DIFdelbegin \DIFdel{observed for the other parameterswhich exhibit a bias in the nested sampling inference. }%DIFDELCMD < \newline 
%DIFDELCMD < 
...
\DIFdel{The additional modulations from $y_{\rm pulsar}$ influence the amplitude of the resulting interference wave, but do not affect its frequency. When we model this data using the Kalman filter without the pulsar terms, the additional amplitude modulations from $y_{\rm pulsar}$ must be described by the single wave model, $y_{\rm Earth}$. As a result the $y_{\rm Earth}$ model must try to compensate for these amplitude modulations by adjusting the parameters which govern the amplitude of the single wave. Consequently the parameters which are most affected are those which determine the amplitude of the GW. Terms which influence the amplitude strongly, such as $h_0$ and $\iota$, are strongly affected and exhibit a strong bias . Terms which influence the amplitude weakly such as $\psi$ exhibit a weaker bias. The wave amplitude is independent of the frequency and so $\Omega$ exhibits no bias. Since a shift in phase affects the instantaneous amplitude of the wave, $\Phi_0$ also exhibits a small bias, despite $A$ not being a function of $\Phi_0$. This hierarchy of biases for each parameter is also what we observe in Figure \ref{fig:parameter_space}, where $\alpha$ and }\DIFdelend \DIFaddbegin \DIFadd{obtained for other parameters, e.g. }\DIFaddend $\delta$\DIFdelbegin \DIFdel{are strongly over-constrained whilst $\Omega$ is well-estimated. }%DIFDELCMD < \newline  
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In order to ascertain how important the bias is for PTA }\DIFdelend \DIFaddbegin \DIFadd{. A discussion on why the bias is present in particular elements of $\boldsymbol{\theta}_{\rm gw}$, and the implications of the bias for }\DIFaddend continuous wave searches \DIFdelbegin \DIFdel{requires a thorough exploration of the astrophysical parameter space which we do not carry out in this work. The magnitude of the bias will vary for different points in the parameter space, as well as for different PTA configurations. If the bias is sufficiently modest then for quiet continuous wave sources with low SNR the uncertainty in the one-dimensional marginalised posterior may dominate and the bias may not be important for searches. This is what we observe for our synthetic data (e.g. Figure \ref{fig:corner_plot_2}); the decreased SNR manifests as a broader marginalised posterior which generally overwhelms the shift due to the bias. Conversely if the bias is sufficiently strong, or the source sufficiently loud with low uncertainties, then the bias may come to systematically affect the inferred parameters. }\DIFdelend \DIFaddbegin \DIFadd{is presented in Appendix \ref{sec:bias_appendix}. }\DIFaddend \newline 


We \DIFdelbegin \DIFdel{have shown }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend in this section \DIFdelbegin \DIFdel{how the dropping of }\DIFdelend \DIFaddbegin \DIFadd{that dropping }\DIFaddend the pulsar terms \DIFdelbegin \DIFdel{results in a bias in }\DIFdelend \DIFaddbegin \DIFadd{biases several of }\DIFaddend the inferred parameters \DIFaddbegin \DIFadd{(e.g.\ those that affect the GW amplitude)}\DIFaddend , not just in the sky \DIFdelbegin \DIFdel{localisation parameters  \mbox{%DIFAUXCMD
\citep[e.g.][]{Zhupulsarterms,Chen2022}}\hskip0pt%DIFAUXCMD
but additionally all parameters which influence the wave amplitude. Whilst the bias is generally small and does not prevent the recovery of accurate posteriors for our example system, it is present and may need to be accounted for for analysis in other parts of the parameter space. This bias is not }\DIFdelend \DIFaddbegin \DIFadd{position  \mbox{%DIFAUXCMD
\citep[e.g.][]{Zhupulsarterms,Chen2022}}\hskip0pt%DIFAUXCMD
. The bias is not }\DIFaddend particular to our method\DIFdelbegin \DIFdel{but a shared feature across }\DIFdelend \DIFaddbegin \DIFadd{; it is shared by }\DIFaddend likelihood based methods that do not include the pulsar terms in the inference model. \DIFdelbegin \DIFdel{The inclusion of the pulsar terms in the methods presented in this work in order to alleviate }\DIFdelend \DIFaddbegin \DIFadd{Generalizing the Kalman tracker and nested sampler to correct for }\DIFaddend the bias will be a key goal of a future \DIFdelbegin \DIFdel{work}\DIFdelend \DIFaddbegin \DIFadd{paper}\DIFaddend . 
...
%DIFDELCMD < %%%
%DIF < https://arxiv.org/pdf/2107.03047.pdf
\section{\DIFdel{Conclusion}}%DIFAUXCMD
...
\DIFdelbegin \section{\DIFdel{Identifiability}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{appendix_identifiability}
%DIFDELCMD < %%%
\DIFdel{\textcolor{red}{TK TBD. An example calculation for a simple system using the methods of e.g. } \mbox{%DIFAUXCMD
\citep{KARLSSON2012941} }\hskip0pt%DIFAUXCMD
\mbox{%DIFAUXCMD
...
\section{\DIFadd{Bias from dropping the pulsar terms}}\label{sec:bias_appendix}
\DIFadd{Why does dropping the pulsar terms bias the above parameters in particular, whilst leaving others nearly unaffected? For instance, why is the bias in $\iota$ so strong, whereas $\psi$ shows only a weak bias and  $\Omega$ shows no bias at all? When we generate synthetic data including the pulsar term, the state frequency of an individual pulsar $f_{\rm p}$ is effective modulated by the interference of two equal-amplitude, phase shifted waves (c.f. Equation \eqref{eq:measurement}). Explicitly we have an 

...
THe standard method of detecting single source psr is as follows

drawback is does not account for noises, ooptimal estimators etc etc. 
...
This is how we usually search for single source PTAs

estimate all unknown parameters optimally
here is a difference approach 
...
\end{align}
which are in turn specified via the location of the GW source on the sky (via polar coordinate $\theta$ and azimuthal coodinate $\phi$) and the polarisation angle $\psi$
\begin{align}

...
The large degree of variance in the 1D posteriors of $\iota$ and $h_0$ is due to the nested sampling algorithm effectively trading off accuracy in one parameter against the accuracy in other. From Equations \ref{eq:hphx}, \ref{eq:hphx2} we can see that there is a weak degeneracy between $\iota$ and $h$. For example, from Equation \eqref{eq:hphx2} it is not clear if a larger value of the cross polarisation strain $h_{\times}$ is due to the system having a larger strain amplitude $h_0$, or a different inclination $\iota$. 




The situation can be further elucidated if we consider instead the alternative parametrisation in terms of $h_{+}$ and $h_{\times}$. From Equation \ref{eq:hij} the amplitude of the plane GW perturbation at a particular sky location $\boldsymbol{n}$ is a linear combination of $h_{+}$ and $h_{\times}$. 




he log-likelihood surface in $\iota -h_0$ space plateaus across a broad range of physically reasonable values.  
...
This degeneracy is broken generally by the relation for $h_{+}$, Equation \eqref{eq:hphx}. Whilst this ensures that both parameters are then generally identifiable formally, in practice an identifiability issue persists since t
...
Since we are in the high SHR regime (c.f. Fgiure X for h = 1012) the posteriors are narrow high confidence in narrow range of values.

In this case we are justified in talking abotu the mode of posteriors

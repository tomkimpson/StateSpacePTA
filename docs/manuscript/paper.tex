...
\begin{abstract}	
	\begin{itemize}
		...
		\item We show how to combine a Kalman filter and nested sampling for Bayesian inference of the model parameters and model selection.
		\item The method is tested on synthetic, astrophysically representative data and shown to be able to estimate the full posterior distributions of all parameters of the model, as well calculating the marginalized likelihood for model selection.
		\item The bias introduced by dropping the `pulsar term' from the state-space model is analysed and discussed. 
	\end{itemize}

...
%\begin{figure*}%
%	\centering
%	\subfloat[\centering $f_{\rm em}^{(n)}(t_1)$]{{\includegraphics[width=0.3 \textwidth]{images/example_residuals_plot} }}%
%	\qquad
%	\subfloat[\centering $\dot{f}_{\rm em}^{(n)}(t_1)$]{{\includegraphics[width=0.3 \textwidth]{images/example_residuals_plot} }}%
%	\qquad
%	\subfloat[\centering $\sigma^{(n)}$]{{\includegraphics[width=0.3\textwidth]{images/example_residuals_plot} }}%
%	
...
The above discussion was just for a single realisation of the noise processes. It is important to confirm that our method is robust against different realisations of the noise and that the specific realisation of the noisy data used in Sections \ref{sec:parameter_estim}, \ref{sec:detection} is not particularly advantageous or convenient for our method.  



To this end we take our representative example from Table \ref{tab:parameters_and_priors} and generate 1000 realisations of the process noise and measurement noise. 


For each realisation of the data we can then independently estimate the static parameters $\boldsymbol{\theta}$.



The estimated values of $\boldsymbol{\theta}_{\rm gw}$ for 5 realisations of the noise are shown in Fig \ref{fig:corner_plot_2} which is analogous to Fig \ref{fig:corner_plot_1} but now shows multiple realisations of the noise. 

We can see that the results agree well with the case of a single noise realisation. The estimates of the parameters $\Omega, \Phi_0, \psi, \delta$ and $\alpha$ are highly consistent across the different realisations, agreeing with the injected values with high fidelity. 


The variance in the estimates of $\iota$ and $h_0$ is much greater than for the other parameters. 



This is due to the nested sampler effectively trading off accuracy in one parameter against the other; from Eq \ref{eq:hphx} \ref{eq:hphx2} we can see that there is a weak degeneracy between $\iota$ and $h$.  For example, from Eq \ref{eq:hphx2} it is not clear if a larger $h_{\times}$ is due to a larger strain $h$ or a different inclination $\iota$. This degeneracy is broken by the relation for $h_{+}$ which does separate $h$ and $\iota$ 

Whilst both parameters are then generally identifiable, the specific algebraic relation between the two and how these parameters get mapped into the measurement equation makes it difficult for our algorithm to estimate both simultaneously, instead typically trading off one parameter against the other. 
...
If we fix one of these parameters, it is then possible for our approach to recover the other parameter with much higher accuracy. This is demonstrated in Fig X which is analogous to the representative exxample, but now we have fixed $h$ at its true value (i.e. reduced the prior to a delta function). WE can see that the method can now infer $\iota$ which much greater accuracy. 
...
In order to visualise the 1000 realisations, it is convenient to consider the distribution in the median of the posterior that is inferred for each static parameter, for each realisation of the noise. The resulting distributions are presented in Fig \ref{fig:median_distriubutins}
...
The bias that was evident for some of the parameters 
  ...
  \textcolor{red}{with the estimate on iota}
...
are entirely consistent with the single


, which is analogous to Fig Y but with multiple  now shown. 
...
\begin{figure*}
	...
	\begin{subfigure}[b]{0.22\textwidth}
		\includegraphics{images/distribution_omega_gw}
		\caption{$\Omega$}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.22\textwidth}
		\includegraphics{images/distribution_phi0_gw}
		\caption{$\Phi_0$}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.22\textwidth}
		\includegraphics{images/distribution_psi_gw}
		\caption{$\psi$}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.22\textwidth}
		\includegraphics{images/distribution_iota_gw}
		\caption{$\iota$}
	\end{subfigure}
	
	\medskip
	\begin{subfigure}[b]{0.22\textwidth}
		\includegraphics{images/distribution_delta_gw}
		\caption{$\delta$}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.22\textwidth}
		\includegraphics{images/distribution_alpha_gw}
		\caption{$\alpha$}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.22\textwidth}
...
more stringent priors on f and fdot 
Points to discuss:

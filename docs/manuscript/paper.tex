...
\begin{abstract}
	\textcolor{red}{TK: will obviously drop the bullet points, but useful for writing a concise abstract.}
	
	\begin{itemize}
		\item Continuous nHz gravitational waves from individual supermassive black hole binaries are a primary target for Pulsar Timing Array (PTA) observations.
		\item We present a new and complementary state-space approach to PTA data analysis.
		\item We show how to combine a Kalman filter and nested sampling to estimate te
 ...
 using the combination of a Kalman filter and .  





that is able to self consistenyl track the intrinsic timingnoise in PTA pulsars and disentangle it from the GW-induced TOA modulat


We illustrate how our method is able to recover the full posterior distributions of the N model parameters 
...
\textbf{MAYB ESUPRESS N ? rm \,}


\textbf{Introduce puslar terms dropping here insttea?}
...
\begin{enumerate}[wide = 0pt]
	\item Sample from the prior $\pi(\boldsymbol{\theta})$ to obtain $\pi(\boldsymbol{\theta})_{\rm sample}$
	\item Pass $\pi(\boldsymbol{\theta})_{\rm sample}$ to the Kalman filter
	...
	\item Repeat
\end{enumerate}
In Section \ref{sec:testing} we will apply this workflow on a synthetic data set.
...
\textbf{CLARIFY re uninformative c.f loguniform}

\textbf{sin/cos priors realy uniform over cos(x)}

\textbf{referecne NANOgrav puslars and n index c.f. TAble ANTF}

\textbf{why did you choose NANOGrav you dick}

\textbf{priors on f, fdot, shrink to e.g. factor x ANTF error}
...
We can see from Figure \ref{fig:corner_plot_2} that the results from these 9 noise realisations agree well with the single realisation result that we saw in  Figure \ref{fig:corner_plot_1} for a single noise realisation. The estimates of the parameters $\Omega, \Phi_0, \psi, \delta$ and $\alpha$ are highly consistent across the different noise realisations with the injected value contained in the 90 \% credible interval for each of the marginalized posteriors. The variance between each of the individual 1D histograms is also generally low. 
...
\begin{itemize}
	...
	\item Lets preempt reviewer and make it explicit WHY we are not using TOAs
\end{itemize}

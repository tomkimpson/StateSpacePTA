...
\end{eqnarray}
We can then vary $\sigma^{(n)}$ and compare the synthetically generate phase residuals with the phase solution from the UTMOST pulsar timing program with the Mongolo Observatory Synthesis Telescope  \citep{UTMOST}. It is sufficient to simply compare the synthetic and real solutions visually, rather than e.g. defining some quantitative loss function (e.g. mean-squared error) between the two solutions and solving the optimisation problem of the value of $\sigma_{\rm TOA}^{(n)}$ which minimised this loss function; we emphasise that we are not overly concerned with getting the maximally accurate values for the noise in the pulsar frequency evolution, but just instead some reasonable, representative values. This approach of generating synthetic phase solutions and visually comparing with real solutions is also followed in \cite{Vargas}. In Figure \ref{fig:qualitative_compare} we demonstrate for a particular representative pulsar, PSR XXX, blah blah blah...
\begin{figure}
	\includegraphics[width=\columnwidth]{images/example_residuals_plot}
	\caption{Actual and synthetic phase evolution}
	\label{fig:qualitative_compare}
\end{figure}


\subsubsection{Synthetic data generation}
We generate $N$ synthetic noisy timeseries by integrating the state equations, Eq \ref{eq:spinevol}- \ref{eq:xieqn} numerically, mapping from state space to measurement space via the measurement equation, Eq. \ref{eq:measurement}, and finally adding to each timeseries some zero-mean Gaussian noise with covariance $\sigma_{\rm m}^2$. We take $\sigma_{\rm m}^2$ to be known \textit{a priori} rather than a parameter to be inferred. Throughout this work we fix $\sigma_{\rm m}^2 = 10^{-11}$. Equations \ref{eq:spinevol}- \ref{eq:xieqn} are solved by a Runge-Kutta It$\hat{\text{o}}$ integrator implemented in the \texttt{sdeint} python package \footnote{\url{https://github.com/mattja/sdeint}}. The static pulsar parameters  $\boldsymbol{\theta}_{\rm psr}$ are completely specified by our construction of the synthetic PTA outlined in Section \ref{sec:synt_pta}. For this work we consider all pulsars to be observed for $T_{\rm obs} =10$ years, uniformly sampled with a weekly cadence. In practice, in order to avoid numerical precision issues that arise when solving Eq \ref{eq:spinevol}- \ref{eq:xieqn} $\left ( \text{since } \sigma^{(n)} \ll f_{\rm p}^{(n)} \right)$ we first ``heterodyne" our state by subtracting the deterministic evolution, equivalent to a change of variables:
\begin{equation}
...
\subsection{Practical considerations for inference}

heterodyeing 

pulsar terms vs earth terms 

For this initial demonstrative work we use only the Earth terms

loss of SNR, cite Zhu etc.

We defer the inclusion of the pulsar terms for a future work 

reduces the dimensionality of the problem to $7 + 4N$ since the measurement equation eq X is no longer a function of the pulsar distance. 


we emphasise that whilst our alternative model $m$ disregards the pulsar terms, the synthetic data does have the pulsar terms

\subsection{PTA pulsars}\label{sec:pta_pulsars}

\noindent With our Kalman filter and nested sampling techniques in hand, 
...
we consider the pulsars to have data over the entire 

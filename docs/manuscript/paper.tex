...
\end{eqnarray}
For the synthetic NANOGrav PTA depicted in Figure \ref{fig:pulsar_distrib}, the median $\sigma^{(n)}$ calculated in this way is $\sigma^{(n)} = 4.28 \times 10^{-21} $ s$^{-5/2}$, $\min [ \sigma^{(n)} ] = 1.3 \times 10^{-23}$s$^{-5/2}$ for PSR J0645+5158 and $\max [ \sigma^{(n)} ] = 2.0 \times 10^{-16}$ s$^{-5/2}$ for PSR J1939+2134 \newline 

As a cross-check, we calculate $\sigma^{(n)}$ by solving Equation \eqref{eq:spinevol} numerically using the \texttt{baboo} package \footnote{\url{https://github.com/meyers-academic/baboo}}, generating a synthetic phase solution,
\begin{eqnarray}
